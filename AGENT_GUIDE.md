...
### Q: select_template 的参数名和节点输入名不一致怎么办？
不需要担心。`select_template` 内置了参数映射，会自动把常见参数名映射到正确的节点输入：

| 你传的参数 | 实际映射到的节点输入 |
|-----------|-------------------|
| `positive_prompt` | `CLIPTextEncode.text`（找 title 含 "Positive" 的节点） |
| `negative_prompt` | `CLIPTextEncode.text`（找 title 含 "Negative" 的节点） |
